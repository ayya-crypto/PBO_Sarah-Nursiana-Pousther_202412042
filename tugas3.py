...
class Peminjaman:
    ...
    def info_peminjaman(self):
        return f"[Kode: {self.kode_buku}] | Pinjam: {self.tanggal_pinjam} | Kembali: {self.tanggal_kembali} | Status: {self.status}"
...
class Anggota:
    ...
    # Getter Daftar Status Anggota (private)
    def get_status(self):
    ...
    # Pinjam Buku
    def pinjam_buku(self, Buku, kode_buku, tanggal_pinjam, tanggal_kembali):
        ...
        if Buku.kurangi_stok(1): # Hanya kurangi stok 1 untuk 1 peminjaman
            # Buat objek peminjaman
            pem = Peminjaman(kode_buku, tanggal_pinjam, tanggal_kembali)
            self.daftar_peminjaman.append(pem)
            print(f"{self.nama} berhasil meminjam buku '{Buku.judul}'")
        else:
            print(f"Peminjaman gagal untuk buku '{Buku.judul}'. Stok tidak cukup.")
    ...
    def info_anggota(self):
        print(f"--- Info Anggota ---")
        print(f"ID: {self.id_anggota}, Nama: {self.nama}, Status: {self.__status_aktif}, Max Pinjam: {self._maks_pinjam}")
...
if __name__ == "__main__":
    ...
    buku3 = perpustakaan.tambah_buku("Database", "Harry", "B003", 4, "C2")

    # 2 Anggota
    # (id_anggota, nama, maks_pinjam)
    a1 = Anggota("A001", "Nana", 2) 
    a2 = Anggota("A002", "Budi", 3)

    print("--- Aksi Peminjaman ---")
    # Peminjaman buku
    # pinjam_buku(self, Buku, kode_buku, tanggal_pinjam, tanggal_kembali)
    a1.pinjam_buku(buku1, buku1.kode_buku, "2024-11-01", "2024-11-08") # Nana pinjam B001
    a1.pinjam_buku(buku2, buku2.kode_buku, "2024-11-02", "2024-11-11") # Nana pinjam B002
    a2.pinjam_buku(buku3, buku3.kode_buku, "2024-11-03", "2024-11-12") # Budi pinjam B003
    print("-------------------------")

    print("--- Batas Peminjaman Nana ---")
    # Coba pinjam lagi (akan gagal karena batas max_pinjam = 2)
    a1.pinjam_buku(buku3, buku3.kode_buku, "2024-11-04", "2024-11-15")
    print("-------------------------")

    # Pengembalian
    print("--- Pengembalian ---")
    # a1 kembalikan buku B001
    a1.kembalikan_buku("B001") 
    print("--------------------")

    # Tampilkan Informasi
    perpustakaan.info_semua_buku() # Lihat stok buku setelah pinjam/kembali
